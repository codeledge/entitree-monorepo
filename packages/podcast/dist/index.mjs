...
async function getSpotifyShowEpisodes(playlistId, access_token, afterDate) {
  ...
  } while (nextPageToken && (!afterDate || DateTime.fromISO(items[items.length - 1].pubDate) > afterDate));
  return items;
}
...
async function getItunesShowEpisodes(itunesId) {
  ...
  const results = (await axios2.get(
...
// src/itunes/types.ts
import { z } from "zod";
var ItunesEpisodeSchema = z.object({
  previewUrl: z.string(),
...
async function convertFeedEpisodeToWikidata(episode, podcast) {
  ...
  if ((_c = episode.image) == null ? void 0 : _c.url) {
    claims[WD_PODCAST_LOGO_URL] = [(_d = episode.image) == null ? void 0 : _d.url];
  }
...
async function renameClaimIdtoLabel(episodes) {
  episodes.map((item) => {
    ...
    for (let key in item.claims) {
      claims[WIKIDATA_LABELS_EN[key]] = item.claims[key];
    }
    item.claims = claims;
  });
  return episodes;
...
import {
  WD_PODCAST_LOGO_URL as WD_PODCAST_LOGO_URL2,
  WD_WEB_FEED_URL,
  getWikidataSparql,
  WD_SPOTIFY_SHOW_ID,
...
WHERE 
{
  ...
  OPTIONAL { ?item wdt:${WD_PODCAST_LOGO_URL2} ?logo. }  
